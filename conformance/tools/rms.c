...
int main(int ac, char *av[])
{
    ...
    if(ac == 4)
    {
        in_file1 = OpenWav(av[1], &tmp_i, &tmp_s, &tmp_i, &bps);
        CloseWavIn(in_file1);
...
void printResult(char *inputFilename1, char *inputFilename2, int totalSamples1, float diffMax, float rms, float ssnr, int segmentLength, int differentSamples, float maxDiffThr, float maxRmsThr)
{
    ...
    int  rmsReached = 0;
    unsigned int tmp_i = 0;
    short tmp_s = 0, bipsIn1 = 0;
    WAVEFILEIN *in_file1;

    in_file1 = OpenWav(inputFilename1, &tmp_i, &tmp_s, &tmp_i, &bipsIn1);
    CloseWavIn(in_file1);
...
void calculateRms(char *inputFilename1, char *inputFilename2, int *differentSamples, int *totalSamples1, int *totalSamples2, double *rmsOut, float *maxDiffOut)
{
    ...
    while(1)
    {
        ...
        ReadWavInt(in_file1, sample_buf1, nSamples1 * nChannels1, &nSamplesRead1);
        ReadWavInt(in_file2, sample_buf2, nSamples2 * nChannels2, &nSamplesRead2);
...
void calculateSegmentalSnr(char *inputFilename1, char *inputFilename2, float *ssnrOut)
{
    ...
    while(1)
    {
        ...
        ReadWavInt(in_file1, sample_buf1, segmentLength, &nSamplesRead1);
        ReadWavInt(in_file2, sample_buf2, segmentLength, &nSamplesRead2);
...
{
    printf("    RMS tool to calculate RMS, max. abs. difference and segmental SNR value between two wave files.\n");
