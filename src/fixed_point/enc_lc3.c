                            ...
                            Word8 *scratchBuffer, int bfi_ext)
{
    ...
    process_olpa_fx(&h_EncSetup->olpa_mem_s6k4_exp, h_EncSetup->olpa_mem_s12k8, h_EncSetup->olpa_mem_s6k4, &pitch,
                    s_12k8, s_12k8_len, &normcorr, &h_EncSetup->olpa_mem_pitch, h_EncSetup->resamp_exp,
    ...
    IF (lsbMode == 0)
    {
        processResidualCoding_fx(d_fx_exp, d_fx,
                                 q_d_fx16,
                                 gain, gain_e, encoder->yLen, h_EncSetup->targetBitsQuant, nBits, resBits, &numResBits
        );
    }
    ELSE
    {
